    ...
    QWidget(parent)
{
    ...
    //默认视野在中间,强制调至左上角，否则无法实现鼠标全局坐标和view相对坐标的对应
    plView->setBackgroundBrush(QImage(":/sea.jpg"));
    plView->setCacheMode(QGraphicsView::CacheBackground);
    //窗口静态布局初始化
    {
        QFont font;
        ...
        coordinateLabel =new QLabel(tr("LON\t\t\t\t\t\tLAT"));
